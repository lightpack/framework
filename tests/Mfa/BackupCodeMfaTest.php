...
class BackupCodeMfaTest extends TestCase
{
    ...
    {
        $this->user->mfa_backup_codes = json_encode(['code1', 'code2']);
    ...
    public function testValidateRemovesCodeAndReturnsTrue()
    {
        ...
        $this->user->mfa_backup_codes = json_encode($hashCodes);
        $result = $this->factor->validate($this->user, 'def456');
        $this->assertTrue($result);
        $remaining = json_decode($this->user->mfa_backup_codes, true);
