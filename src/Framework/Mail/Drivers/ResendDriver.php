...
class ResendDriver implements DriverInterface
{
     ...
     * 
     * @param array $emails Array of normalized email data
     * @return array Results with email IDs or errors
     */
    public function sendBatch(array $emails): array
    {
        try {
            // Transform each email to Resend format
            $batch = [];
            ...
            $result = $this->resend->batch->send($batch);
            
            return $result['data'] ?? [];
        } catch (\Exception $e) {
            throw new GlobalException("Resend Batch API Error: {$e->getMessage()}");
        }
    }
}
