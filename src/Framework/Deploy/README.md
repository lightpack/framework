...
Lightpack Deploy is a set of commands built into the framework that handle the boring, repetitive, and error-prone parts of running a modern web application in production:
...
The bash script `src/Framework/Deploy/scripts/provision.sh` is missing from your installation. It ships with the framework, so this should not happen unless files were deleted.
...
Lightpack Deploy is built on a few simple beliefs:
